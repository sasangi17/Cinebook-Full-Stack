...
const Dashboard = () => {
    ...
    const dashboardCards=[
        {title:"Total Bookings",value: dahboardData.totalBookings||"0",icon:ChartLineIcon},
        {title:"Total Revenue",value: currency+dahboardData.totalRevenue||"0",icon:CircleDollarSignIcon},
        {title:"Active Shows",value: dahboardData.activeShows.length||"0",icon:PlayCircleIcon},
        {title:"Total Users",value: dahboardData.totalUser||"0",icon:UsersIcon}
  ...
  return !loading? (
    ...
    <div className='relative flex flex-wrap gap-6 mt-4 max-w-5x1'>
        {dahboardData.activeShows.map((show)=>(
            <div key={show._id} className='w-55 rounded-lg overflow-hidden h-full pb-3 bg-primary/10 border border-primary/20 hover:-translate-y-1 transition duration-300'>
                <img src={show.movie.backdrop_path} alt='' className='h-60 w-full object-cover'/>
